...
/*
// Type: instruction_t 操作 operation
// ------------------
...
/*
// Type: species_t 物种：名字，可执行的操作*数量
// ------------------
...
/*
// Type: creature_t 生物个体：位置 方向 物种 现行操作ID 能力（比如 ability[FLY]，仅有两个选项） hillActive 是表示如果次轮在山上，是否轮空 
// ------------------
...
struct grid_t
{
    unsigned int height;
    unsigned int width;
    creature_t *squares[MAXHEIGHT][MAXWIDTH];  //生物网络分布 来自world-test 文件
    terrain_t terrain[MAXHEIGHT][MAXWIDTH];		//地形分布
};
/*
// Type: grid_t
// ------------------
...
struct world_t
{
    ...
    species_t species[MAXSPECIES];      //生物种类 即tests/species文件

    unsigned int numCreatures;
    creature_t creatures[MAXCREATURES]; //生物，存储trun执行次序
